...
import Breadcrumb from '../../components/plm/Breadcrumb';
...
const DocumentDetailPage = () => {
  ...
  if (loading) return <div className="ddp__loading"><div className="ddp__spin" /><span>Loading…</span></div>;
  if (!doc)    return <div className="ddp__err-page">Document not found. <button onClick={() => navigate(-1)}>Go back</button></div>;

  const transitions = PROMOTE_MAP[doc.lifecycleState] ?? [];
  const isCheckedOutByMe = doc.checkedOutBy === user?.username;
    ...
    <div className="ddp">
      ...
      <div className="ddp__info-hdr">
        ...
        <div className="ddp__actions">
          {/* Promote buttons */}
          {transitions.map(t => (
            <button key={t} className="ddp__act-btn" disabled={busy || isCheckedOut}
          ...
          {/* Checkout / checkin */}
          {!isCheckedOut && (
            <button className="ddp__act-btn ddp__co-btn" disabled={busy}
              onClick={() => act(() => documentApi.checkOut(doc.id), 'Check out')}
            >🔒 Check Out</button>
          )}
          {isCheckedOutByMe && (
            <>
              <button className="ddp__act-btn ddp__ci-btn" disabled={busy}
                onClick={() => act(() => documentApi.checkIn(doc.id), 'Check in')}
              >✅ Check In</button>
              <button className="ddp__act-btn ddp__undo-btn" disabled={busy}
...
const RelatedParts = ({ docId }) => {
  ...
  useEffect(() => {
    documentApi.byPart(docId)  // re-using listByPart endpoint: /by-part/{partId}
      // Actually we need parts-for-doc endpoint; use the inverse: list docs for each known part.
      // For now render nothing until a GET /by-doc/{docId} endpoint is added.
      .catch(() => {})
      .finally(() => { setParts([]); setLoading(false); });
  }, [docId]);
  ...
  return (
    <div className="ddp__rp">
      {parts.length === 0
        ...
        : parts.map(p => (
            <div key={p.id} className="ddp__rp-item" onClick={() => navigate(`/plm/parts/${p.id}`)}>
