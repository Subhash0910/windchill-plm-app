...
export const plmApi = {
  ...
  /** Global audit feed — GET /api/v1/plm/audit/all?limit=N */
  ...
  // ── Changes (ECR) ───────────────────────────────────────
  createEcr: async (p) => { const r = await api.post('/api/changes/ecr', p); return r.data; },
  submitEcr: async (id, p) => { const r = await api.post(`/api/changes/ecr/${id}/submit`, p); return r.data; },
  listEcrs: async (status) => { const r = await api.get('/api/changes/ecr', { params: status ? { status } : {} }); return r.data; },
  getEcrDetails: async (id) => { const r = await api.get(`/api/changes/ecr/${id}/details`); return r.data; },
  getEcrInsights: async (id) => { const r = await api.get(`/api/changes/ecr/${id}/insights`); return r.data; },
  analyzeEcr: async (id) => { const r = await api.post(`/api/changes/ecr/${id}/analyze`); return r.data; },

  // ── Change Tasks ─────────────────────────────────────────
  getMyChangeTasks: async (pendingOnly = true) => { const r = await api.get('/api/changes/tasks/my', { params: { pendingOnly } }); return r.data; },
  approveChangeTask: async (id, comment) => { const r = await api.post(`/api/changes/tasks/${id}/approve`, { comment: comment || '' }); return r.data; },
  rejectChangeTask: async (id, comment) => { const r = await api.post(`/api/changes/tasks/${id}/reject`, { comment: comment || '' }); return r.data; },
